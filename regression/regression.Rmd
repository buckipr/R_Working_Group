...
---
# Linear regression: margins

Predicted values of prestige ( $\hat{y}$ ) using the `margins` package

```{r, echo=TRUE, eval=TRUE}
p1 <- prediction(model1,
  at = list(occ = c("blue_collar", "professional", "white_collar")))
summary(p1)
```
...
quasibinomial(link = "logit")  # negbinom?
quasipoisson(link = "log")
```

* Links include: "logit", "probit", "cauchit", "cloglog", "identity", "log", "sqrt", "1/mu^2", "inverse"
...
* check out the [effects package](https://cran.r-project.org/web/packages/effects/index.html)?

* comparing regression models (nested and non-nested)
