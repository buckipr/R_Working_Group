...
* Plan for this semester:  <br> build R into your workflow (even if not for analysis)
...
setwd("Thesis/Analysis")     # set the working directory to the folder Thesis/Analysis
...
```{r, load_dplyr, eval=TRUE, echo=FALSE, include=FALSE}
library(dplyr)
```


---
## **Data Structures**: `dplyr` select columns
...
* Old School way to access a single column in a data frame is to use `$`
